...
const tasklist = (req,res)=>{
    ...
    sql.connect(config,function(err){
       ...
       // var query = "SELECT Task.alarmid, Task.taskname, Task.taskassigneddate as assigneddate, Task.taskpriority as priority, Task.taskrepeatdays as repeated, Alarm.deviceid, Task.taskid, Task.buildingname, Task.taskcloseddate,Task.closingdesc, Task.escalationstage  FROM [ECCDB].[ECCAnalytics].[Alarm] INNER JOIN [ECCDB].[ECCAnalytics].[Task] ON Alarm.alarmid = Task.alarmid;"
   /*    var query = "SELECT [T1].[taskid],[T1].[subequipmenttype],[T6].equipmenttype, [T1].[taskname],[T4].[cityname],[T3].[campusname],[T1].[taskassigneddate],[T1].[taskpriority],[T1].[taskcloseddate],[T1].[taskcloseddesc],[T1].[taskrepeatdays],[T1].[escalationstage], [T2].[buildingname] FROM [ECCDB].[ECCAnalytics].[Task] T1"
       query += " inner join [ECCDB].[ECCAnalytics].[Buildings] T2 " 
       query +=" on [T1].buildingname = [T2].buildingname "
       query +="inner join [ECCDB].[ECCAnalytics].[Campuses] T3 "
       query +="ON [T3].[campusname] = [T2].[campusname] "
       query +="inner join [ECCDB].[ECCAnalytics].[Cities] T4 "
       query +="on [T3].[cityname] = [T4].[cityname] "
       query +=" inner join [ECCDB].[ECCAnalytics].[Countries] T5"
       query +=" on [T4].[countryname] = [T5].[countryname] " 
       query += " inner join [ECCDB].[ECCAnalytics].[SubEquipments] T6 "
...
const taskDetailsByAlarmId = (req,res)=>{
    ...
    sql.connect(config,function(err){
       ...
       var query = "SELECT  [recordid] ,[taskid] ,[alarmid] ,[taskname],[taskpriority],[taskassigneddate],[taskcloseddate],[taskcloseddesc],[taskrepeatdays],[feedback],[buildingname],[subequipmenttype],[closingdesc],[escalationstage],[taskstatus]  FROM [ECCDB].[ECCAnalytics].[Task] T1 where [T1].alarmid = "+aid+""
...
const alarmfilterdata = (req,res)=>{
   ...
   // var query = "SELECT [T1].[alarmid],[T1].[datapointrecordid] ,[T1].[ruleid] ,[T1].[deviceid] ,[T1].[analysisname] ,[T1].[analyticsummary] ,[T1].[measuretype] ,[T1].[costavoided] ,[T1].[energysaved] ,[T1].[alarmstatus],[T1].[alarmontimestamp],[T1].[alarmofftimestamp],[T1].[escalationstage],[T1].[buildingname],[T1].[taskstatus],[T2].[equipmentname] "
    //query += " FROM [ECCDB].[ECCAnalytics].[Alarm] T1 "
    //query += " left join [ECCDB].[ECCAnalytics].[Devices] T2"
  ...
  // query += " on [T9].subequipmentname = [T10].subequipmenttype where  T1.[alarmstatus] != 0 and T1.[taskstatus]  != 0 OR T1.[alarmstatus] = 0 and T1.[taskstatus]  != 0 OR T1.[alarmstatus] != 0 and T1.[taskstatus]  = 0   ORDER by T1.[alarmid] "
    ...
    if (typeof req.body.country !== 'undefined') {
      ...
      /* var query = "SELECT [T1].[alarmid],[T1].[datapointrecordid] ,[T1].[ruleid] ,[T1].[deviceid] ,[T1].[analysisname] ,[T1].[analyticsummary] ,[T1].[measuretype] ,[T1].[costavoided] ,[T1].[energysaved] ,[T1].[alarmstatus],[T1].[alarmontimestamp],[T1].[alarmofftimestamp],[T1].[escalationstage],[T1].[buildingname],[T1].[taskstatus] "
       query += " FROM [ECCDB].[ECCAnalytics].[Alarm] T1 "
        query += " left JOIN [ECCDB].[ECCAnalytics].[Buildings] T2" 
        query += " on [T1].buildingname =  [T2].buildingname"
        query += " left JOIN [ECCDB].[ECCAnalytics].[Campuses] T3" 
        query +=" on [T2].[campusname] = [T3].[campusname]" 
        query +=" left JOIN [ECCDB].[ECCAnalytics].[Cities] T4"
...
const addtask = (req,res)=>{
    ...
    maxsql = "SELECT MAX(recordid) as maxid FROM [ECCDB].[ECCAnalytics].[Task]"
    ...
    sql.connect(config,function(err){
        ...
        request.query(maxsql,function(err,records){
            ...
            console.log(err);
                    ...
                    query2 = "INSERT INTO [ECCDB].[ECCAnalytics].[Task] ([taskid],[alarmid],[taskname],[taskpriority],[taskassigneddate],[buildingname],[subequipmenttype],[taskstatus],[escalationstage]) VALUES ('"+taskid.toString()+"',"+req.body.alarmid.toString()+",'"+req.body.taskname.toString()+"','Low', CURRENT_TIMESTAMP,'"+req.body.buildingname.toString()+"','"+req.body.equipmentname.toString()+"','1','Stage1');"
                    updatequery = "update alarm set [alarm].[taskstatus] = 1 from [ECCDB].[ECCAnalytics].[alarm] alarm  where [alarm].[alarmid] = "+alarmid+";"
...
const countryforcombo = (req,res)=>{
    sql.connect(config,function(err){
       ...
       var query = "SELECT [countryname]  FROM [ECCDB].[ECCAnalytics].[Countries]"
...
const cityforcombo = (req,res)=>{
    sql.connect(config,function(err){
       ...
       var query = "SELECT [cityname]  FROM [ECCDB].[ECCAnalytics].[Cities]"
...
const campusforcombo = (req,res)=>{
    sql.connect(config,function(err){
       ...
       var query = "SELECT [campusname] as campus  FROM [ECCDB].[ECCAnalytics].[Campuses]"
...
const citycampus = (req,res)=>{
    ...
    sql.connect(config,function(err){
       ...
       var query = "SELECT Building.[buildingname],Campuses.[campusname],Cities.[cityname]  FROM [ECCDB].[ECCAnalytics].[Buildings] Building inner join [ECCDB].[ECCAnalytics].[Campuses] Campuses  on Building.[campusname] = Campuses.[campusname] inner join [ECCDB].[ECCAnalytics].[Cities] Cities  on Cities.[cityname] = Campuses.[cityname] where Building.[buildingname] = '"+building+"'"
...
const avgdatapointvalue = (req,res)=>{
    ...
    sql.connect(config,function(err){
       ...
       var query = "SELECT TOP (60) AVG(convert(float,[DataPointValue].[datapointvalue])) as [average-datapointvalue]  FROM [ECCDB].[ECCAnalytics].[DataPointValue] DataPointValue where [DataPointValue].deviceid = '"+deviceid+"' and [DataPointValue].datapoint = '"+datapoint+"'"
...
const closetask = (req,res)=>{
    ...
    query = " update Task set [Task].[taskstatus] = '0',  [Task].[taskcloseddate] = CURRENT_TIMESTAMP, [Task].[taskcloseddesc] = '"+closingdesc+"', [Task].[closingdesc] = '"+closingdesc+"', [Task].[feedback] = '"+feedback+"' from [ECCDB].[ECCAnalytics].[Task] Task where [Task].[alarmid] = "+alarmid+";"
    query2 = " update Alarm set [Alarm].[costavoided] = "+costavoided+", [Alarm].[energysaved] = "+energysaved+", [Alarm].[alarmstatus] = 0, [Alarm].[alarmofftimestamp] = CURRENT_TIMESTAMP,  [Alarm].[taskstatus] = '2' from [ECCDB].[ECCAnalytics].[Alarm] Alarm where [Alarm].[alarmid] = "+alarmid+";"
     query3 = " update Ruletimer set [Ruletimer].[timer] = 0 from [ECCDB].[ECCAnalytics].[Ruletimer] Ruletimer where [Ruletimer].[workflowname] = '"+ruleno+"';"
...
const buildingname = (req,res)=>{
    ...
    sql.connect(config,function(err){
       ...
       query +=  "FROM [ECCDB].[ECCAnalytics].[DataPointValue] DataPointValue "
       query +=  "inner join [ECCDB].[ECCAnalytics].[DataPoint] DataPoint on DataPoint.datapointid =  DataPointValue.datapointid "
       query +=  "inner join [ECCDB].[ECCAnalytics].[Devices] Devices on Devices.deviceid = DataPoint.deviceid "
       query +=  "inner join [ECCDB].[ECCAnalytics].[Project] Project on Project.equipmentname = Devices.equipmentname"
...
const email = (req,res)=>{
    ...
    sql.connect(config,function(err){
       ...
       //var query = "SELECT [buildingname] as building FROM [ECCDB].[ECCAnalytics].[Buildings]"
       query = "SELECT [ruleid],[ruleno],[possiblecauses],[recommendations],[duration],[inputname],[alarmemailid],[escalationemailid] FROM [ECCDB].[ECCAnalytics].[Rules] where [Rules].[ruleid] = '"+req.query.ruleid+"';"
