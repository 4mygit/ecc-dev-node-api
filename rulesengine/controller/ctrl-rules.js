...
const projview = (req,res)=>{
    ...
    sql.connect(config,function(err){
       ...
       query = "SELECT [recordid] ,[projname],[projdesc],[countryname],[countrydesc],[cityname],[citydesc],[campusname],[campusdesc],[buildingname],[buildingdesc],[equipmentname],[equipmenttype],[subequipmentname],[subequipmentdesc],[users]  FROM "+DB+".[ECCAnalytics].[Project]"
        request.query(query,function(err,records){
            if(err)
            console.log(err);
            else{
...
//sql = "" 
count = 0
for (const dic of req.body) {  
  qry2Values+="('"+dic['datapointid']+"', '"+dic['deviceid']+"', '"+dic['datapoint']+"', '"+dic['datapointvalue']+"', CURRENT_TIMESTAMP,'"+dic['units']+"'),"
 //console.log(x); 
} 

var qry = "INSERT INTO ["+dbName+"].[ECCAnalytics].DataPointValue ( datapointid,deviceid,datapoint,datapointvalue,dated,units) VALUES " + qry2Values
